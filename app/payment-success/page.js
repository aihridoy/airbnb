...
export default function PaymentSuccess() {
    ...
    const downloadReceipt = async () => {
        ...
        drawDetail('Check-in:', formatDate(booking?.bookingDetails?.checkInDate) || 'N/A');
        drawDetail('Check-out:', formatDate(booking?.bookingDetails?.checkOutDate) || 'N/A');
        ...
        drawDetail('Rating:', `${booking?.bookingDetails?.averageRating?.toFixed(1)} (${booking?.bookingDetails?.totalReviews} review(s))`);
        drawDetail('Card Number:', booking?.paymentDetails?.cardNumber || 'N/A');
        drawDetail('Billing Address:', `${booking?.paymentDetails?.billingAddress?.streetAddress}, ${booking?.paymentDetails?.billingAddress?.city}, ${booking?.paymentDetails?.billingAddress?.state},\n${booking?.paymentDetails?.billingAddress?.zipCode}`);
                    ...
                    <div className="flex items-start gap-6 mb-6 pb-6 border-b">
                        ...
                        <div>
                            ...
                            <div className="flex items-center mb-2">
                                ...
                                <span className="text-sm">{booking?.bookingDetails?.averageRating?.toFixed(1)} ({booking?.bookingDetails?.totalReviews}+ reviews)</span>
                            </div>
                        ...
                        <div>
                            ...
                            <div className="space-y-3">
                                <div className="flex justify-between">
                                    ...
                                    <span className="text-zinc-500 text-sm">{formatDate(booking?.bookingDetails?.checkInDate)}</span>
                                </div>
                                <div className="flex justify-between">
                                    <span className="text-zinc-600 text-sm">Check-out</span>
                                    <span className="text-zinc-500 text-sm">{formatDate(booking?.bookingDetails?.checkOutDate)}</span>
                                </div>
                                <div className="flex justify-between">
                                    <span className="text-zinc-600 text-sm">Guests</span>
                                    <span className="text-zinc-500 text-sm">{booking?.bookingDetails?.guests} guest</span>
                                </div>
                            </div>
                        </div>
